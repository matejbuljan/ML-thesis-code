...
def train_one_epoch(model, optimizer, train_loader, accumulated_iter, optim_cfg,
                    ...
                    logger=None, logger_iter_interval=50, cur_epoch=None, total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300, discriminator_optimizer=None, mtr_loss=None, dis_loss=None):
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if torch.isnan(loss).any() or torch.isinf(loss).any():
            print("NaNs or Infs found in mtr + generator loss")

        loss.backward()
        for name, param in model.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                    print('Found nans in the model itself')
                    model.motion_decoder.problem_counter_mtr += 1
                    print(f'Nans in mtr found {model.motion_decoder.problem_counter_mtr} times')

                    # accumulated_iter += 1
                    # return accumulated_iter
                    # print(f"NaNs or Infs found in gradients of {name}")
        ...
        if model.motion_decoder.training_gan:
            discriminator_loss = model.motion_decoder.get_discriminator_loss()
            if torch.isnan(discriminator_loss).any() or torch.isinf(discriminator_loss).any():
                print("NaNs or Infs found in discriminator_loss")
            # mtr_loss.append(loss)
            # dis_loss.append(discriminator_loss)
            discriminator_optimizer.zero_grad()
            discriminator_loss.backward()
            for name, param in model.motion_decoder.discriminator.named_parameters():
                if param.grad is not None:
                    if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                        print('Found nan in the discriminator gradients')
                        model.motion_decoder.problem_counter_disc += 1
                        print(f'Nans in discriminator found {model.motion_decoder.problem_counter_disc} times')
                        accumulated_iter += 1
                        return accumulated_iter
                        # print(f"NaNs or Infs found in gradients of {name}")

            torch.nn.utils.clip_grad_norm_(model.motion_decoder.discriminator.parameters(), max_norm=1.0)
...
def train_model(model, optimizer, train_loader, optim_cfg,
    ...
    dis_loss = []
    if model.motion_decoder.training_gan:
        discriminator_optimizer = torch.optim.Adam(model.motion_decoder.discriminator.parameters(), lr=0.001, weight_decay=1e-5)
    else:
    ...
    print(f'Number of loss points: {len(mtr_loss)} and {len(dis_loss)}')
