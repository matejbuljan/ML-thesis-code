...
def parse_config():
    ...
    parser.add_argument('--training_gan', action='store_true', default=False, help='')
    args = parser.parse_args()

    print(f'TRAINING GAN {args.training_gan} HERE')
...
def main():
    ...
    training_gan = args.training_gan
    # training_gan = True
    tb_writer = SummaryWriter('/home/mbultc/PycharmProjects/MTR/my_scripts/tensorboard')
    print(f'================ We are training a GAN: {training_gan} with batch size {args.batch_size}============================')
    model = model_utils.MotionTransformer(config=cfg.MODEL, training_gan=training_gan, tb_writer=tb_writer)
