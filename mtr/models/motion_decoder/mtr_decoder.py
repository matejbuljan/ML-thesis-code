...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config, training_gan=False, tb_writer=None):
        super().__init__()
        ...
        self.training_gan = training_gan
        self.timesteps = 91
        self.num_of_features = 10
        self.discriminator = nn.Sequential(
            nn.Linear(self.timesteps * self.num_of_features, 512),
            nn.LeakyReLU(0.1),
            nn.Linear(512, 128),
            nn.LeakyReLU(0.1),
            nn.Linear(128, 32),
        ...
        self.gen_counter = 0
        self.dis_counter = 0
        self.problem_counter_mtr = 0
        self.problem_counter_disc = 0
    ...
    def sigmoid_cross_entropy(self, targets, logits):
        assert targets.shape == logits.shape
        logits = torch.clamp(logits, min=-999, max=999)
        check_before = torch.isnan(logits).any() or torch.isinf(logits).any()
        # Use PyTorch's built-in functions for numerical stability
        loss = torch.nn.functional.binary_cross_entropy_with_logits(logits, targets, reduction='none')
        check_after = torch.isnan(loss).any() or torch.isinf(loss).any()
        if check_before or check_after:
            print(f'Check before: {check_before}')
            print(f'Check after: {check_after}')
        assert loss.shape == logits.shape
        return loss

    def get_generator_loss(self):
        fake_logits = self.gan_dict['fake_logits']
        loss = torch.mean(self.sigmoid_cross_entropy(torch.ones_like(fake_logits), fake_logits))
        return loss

    def get_discriminator_loss(self):
        # We multiply by 2 because these targets and logits include equal amount of both the real and the fake logits
        targets = self.gan_dict['mixed_labels']
        logits = self.gan_dict['mixed_logits']
        loss = 2 * torch.mean(self.sigmoid_cross_entropy(targets, logits))
        self.dis_counter += 1
        self.tb_writer.add_scalar('GAN discriminator loss', loss, self.dis_counter)
        return loss
    ...
    def get_loss(self, tb_pre_tag=''):
        ...
        self.gen_counter += 1
        self.tb_writer.add_scalar('Decoder loss', loss_decoder, self.gen_counter)
        self.tb_writer.add_scalar('Dense prediction loss', loss_dense_prediction, self.gen_counter)
        ...
        if self.training_gan:
            ...
            if torch.isnan(gen_loss).any() or torch.isinf(gen_loss).any():
                print("NaNs or Infs found in generator loss!")
            self.tb_writer.add_scalar('GAN generator loss', gen_loss, self.gen_counter)
            # print(f'Loss decoder: {loss_decoder}, Loss dense_prediction: {loss_dense_prediction}, Loss gan_loss: {gen_loss}')
            total_loss += gen_loss
    ...
    def forward(self, batch_dict):
        ...
        if not self.training:
            ...
            # For GAN, take this batch_dict and send it to waymo_dataset.generate_prediction_dicts()
            # or alternatvely take the code from that function, make a helper function in thi file and have it return
            # processed trajectories so that it returns the ground truth trajectories and the predicted ones in the same format
            # But shouldn't this be done while training, that is in the next branch?
            ...
            if self.training_gan:
                pred_scores, pred_trajs = self.generate_final_prediction(pred_list=pred_list, batch_dict=batch_dict)
                check_pred_trajs_nan = torch.isnan(pred_trajs).any()
                check_pred_trajs_inf = torch.isinf(pred_trajs).any()
                check_pred_scores_nan = torch.isnan(pred_scores).any()
                check_pred_scores_inf = torch.isinf(pred_scores).any()
                if check_pred_trajs_inf or check_pred_trajs_nan:
                    print('We are in the forward method, looking at pred_trajs and scores')
                    print(pred_trajs.shape)
                    print(f'Nans? {torch.isnan(pred_trajs).sum()}')
                    print(f'Inf? {torch.isinf(pred_trajs).sum()}')
                    print(pred_scores.shape)
                    print(f'Nans? {torch.isnan(pred_scores).sum()}')
                    print(f'Inf? {torch.isinf(pred_scores).sum()}')
                    pred_trajs = torch.rand_like(pred_trajs)
                    pred_scores = torch.rand_like(pred_scores)
                    self.problem_counter_disc += 1
                    print(f'Problem counter: {self.problem_counter_disc}')
                batch_dict['pred_scores'] = pred_scores
                batch_dict['pred_trajs'] = pred_trajs
                final_pred_dicts = self.generate_prediction_dict(batch_dict)
                fake_trajs, real_trajs = self.generate_fake_and_real_trajs(final_pred_dicts)
                fake_trajs_detached = fake_trajs.detach()
                mixed_data, mixed_labels = self.process_trajs_for_discrimination(fake_trajs_detached, real_trajs)
                check_mixed_data = torch.isnan(mixed_data).any() or torch.isinf(mixed_labels).any()
                mixed_logits = self.discriminator(mixed_data)
                check_mixed_logits = torch.isnan(mixed_logits).any() or torch.isinf(mixed_logits).any()
                fake_logits = self.discriminator(fake_trajs.reshape(fake_trajs.shape[0], -1))
                self.gan_dict['fake_logits'] = fake_logits
                self.gan_dict['mixed_logits'] = mixed_logits
                self.gan_dict['mixed_labels'] = mixed_labels
                if check_mixed_data or check_mixed_logits:
                    print(f'Check mixed data {check_mixed_data}')
                    print(f'Check mixed logits {check_mixed_logits}')
                    raise Exception


        return batch_dict

    def generate_prediction_dict(self, batch_dict):
        """
        ...
        for bs_idx in range(batch_dict['batch_size']):
            ...
            for obj_idx in range(start_obj_idx, start_obj_idx + batch_sample_count[bs_idx]):
                ...
                check_processed_pred_trajs = torch.isnan(single_pred_dict['pred_trajs']).any() or torch.isinf(single_pred_dict['pred_trajs']).any()
                if check_processed_pred_trajs:
                    print('We are in the generate prediction dict function')
                    print(f'Pred trajs processed: {check_processed_pred_trajs}')
                    raise Exception
    ...
    def generate_fake_and_real_trajs(self, pred_dict):
        updated_trajs_list = []
        gt_trajs_list = []
        for agent_dict in pred_dict[0]:
            check_pred_trajs = torch.isnan(agent_dict['pred_trajs']).any() or torch.isinf(agent_dict['pred_trajs']).any()
            check_gt_trajs = torch.isnan(agent_dict['gt_trajs']).any() or torch.isinf(agent_dict['gt_trajs']).any()
            gt_trajs_list.append(agent_dict['gt_trajs'])
            z = agent_dict['gt_trajs'][
                10, 2]  # This is the z-coordinate which we will keep constant to what it was at the last timestep of the input
            ...
            heading_vector = torch.full((80, 1), heading)
            z_vector = torch.full((80, 1), z)
            velocity_xy = (xy_vector[1:, :] - xy_vector[:-1, :]) * 10
            new_vel_x_vector = torch.full((80, 1), vel_x)
            new_vel_x_vector[1:, :] = velocity_xy[:, 0].unsqueeze(1)
            new_vel_y_vector = torch.full((80, 1), vel_y)
            new_vel_y_vector[1:, :] = velocity_xy[:, 1].unsqueeze(1)
            dx_vector = torch.full((80, 1), dx)
            dy_vector = torch.full((80, 1), dy)
            dz_vector = torch.full((80, 1), dz)
            valid_vector = torch.full((80, 1), valid)

            # Heading calculations
            for timestamp_2s in range(4):
                distance_over_2s = torch.sqrt(
                    (xy_vector[10 * (timestamp_2s + 1), 0] - xy_vector[10 * timestamp_2s, 0]).pow(2) +
                    (xy_vector[10 * (timestamp_2s + 1), 1] - xy_vector[10 * timestamp_2s, 1]).pow(2))
                if distance_over_2s > 0.3:
                    if timestamp_2s > 0:
                        delta_xy_vector = (xy_vector[(timestamp_2s * 20):(timestamp_2s + 1) * 20, :] -
                                           xy_vector[(timestamp_2s * 20 - 1):((timestamp_2s + 1) * 20 - 1), :])
                        heading_vector[(timestamp_2s * 20):(timestamp_2s + 1) * 20] = torch.atan2(
                            delta_xy_vector[:, 1].unsqueeze(1), delta_xy_vector[:, 0].unsqueeze(1))
                        ...
                        heading_vector[1:20] = torch.atan2(delta_xy_vector[:, 1].unsqueeze(1),
                                                           delta_xy_vector[:, 0].unsqueeze(1))
            ...
            agent_updated_trajs = torch.cat((xy_vector.cuda(), z_vector.cuda(), dx_vector.cuda(), dy_vector.cuda(), dz_vector.cuda(), heading_vector.cuda(),
                                             new_vel_x_vector.cuda(), new_vel_y_vector.cuda(), valid_vector.cuda()), dim=1)
            check_updated_trajs = torch.isnan(agent_updated_trajs).any() or torch.isinf(agent_updated_trajs).any()
            agent_full_predicted_traj = torch.cat((agent_dict['gt_trajs'][:11, :].cuda(), agent_updated_trajs))

            updated_trajs_list.append(agent_full_predicted_traj)
            if check_pred_trajs or check_gt_trajs or check_updated_trajs:
                print('We are in the generate fake and real trajs function')
                print(f'Pred trajs unprocessed: {check_pred_trajs}')
                print(f'GT trajs: {check_gt_trajs}')
                print(f'Updated trajs: {check_updated_trajs}')
                raise Exception


        updated_trajs = torch.stack(updated_trajs_list)  # Shape (#center_agents, 91, 10)
        gt_trajs = torch.stack(gt_trajs_list)  # Shape (#center_agents, 91, 10)
        return updated_trajs, gt_trajs

    def process_trajs_for_discrimination(self, fake_trajs, real_trajs):
        number_of_center_agents = fake_trajs.shape[0]
        fake_labels = torch.zeros((number_of_center_agents, 1))
        real_labels = torch.ones((number_of_center_agents, 1))
        labels = torch.cat((fake_labels, real_labels))
        new_order = torch.randperm(labels.shape[0])
        labels = labels[new_order].cuda()
        fake_trajs_flattened = fake_trajs.view(number_of_center_agents, -1)
        real_trajs_flattened = real_trajs.view(number_of_center_agents, -1)
        trajs_flattened = torch.cat((fake_trajs_flattened, real_trajs_flattened.cuda()))
        mixed_trajs = trajs_flattened[new_order]
        return mixed_trajs, labels
