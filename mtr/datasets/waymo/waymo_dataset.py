...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training=True, logger=None, updated_trajs=None, updated_frames=0, updated_scenario_id=None):
        ...
        self.infos = self.get_all_infos(self.data_root / self.dataset_cfg.INFO_FILE[self.mode], scenario_id=updated_scenario_id)
        self.logger.info(f'Total scenes after filters: {len(self.infos)}')

        self.updated_trajs = updated_trajs
        self.updated_frames = updated_frames
        self.updated_scenario_id = updated_scenario_id

    def get_all_infos(self, info_path, scenario_id=None):
        ...
        for func_name, val in self.dataset_cfg.INFO_FILTER_DICT.items():
            infos = getattr(self, func_name)(infos, val, scenario_id=scenario_id)

        return infos

    def filter_info_by_object_type(self, infos, valid_object_types=None, scenario_id=None):
        ret_infos = []
        for cur_info in infos:
            ...
            # Commented below line because list of agents to resimulate for wosac is not the same as the one for mtr
            # cur_info['tracks_to_predict']['difficulty'] = list(np.array(cur_info['tracks_to_predict']['difficulty'])[valid_mask])

            scene_id = cur_info['scenario_id']
            if scenario_id is not None:
                if scene_id == scenario_id:
    ...
    def create_scene_level_data(self, index):
        ...
        track_infos = info['track_infos']       # This needs to be updated when doing closed-loop simulation

        track_index_to_predict = np.array(info['tracks_to_predict']['track_index'])
        obj_types = np.array(track_infos['object_type'])
        obj_ids = np.array(track_infos['object_id'])    # All objects' ids

        obj_trajs_full = track_infos['trajs']  # (num_objects, num_timestamp, 10)
        # print(f'In scene {scene_id} there are {len(track_index_to_predict)} tracks to predict out of {obj_trajs_full.shape[0]}')
        if (self.updated_trajs is not None) and (self.updated_frames > 0) and (self.updated_scenario_id == scene_id):
            # print(f"I'm updating the trajectories of scenario {scene_id}")
            obj_trajs_full = self.update_full_obj_trajs(obj_ids, track_index_to_predict, obj_trajs_full, self.updated_trajs, self.updated_frames)
    ...
    def update_full_obj_trajs(self, obj_ids, track_index_to_predict, obj_trajs_full, updated_trajs, updated_frames):
        remaining_obj_ids = np.delete(obj_ids, track_index_to_predict)
        track_index_non_central_agents = np.where(np.isin(obj_ids, remaining_obj_ids))[0]
        # For central objects, those to be predicted
        updated_trajs_snippet = updated_trajs[:, :updated_frames, :]
        center_obj_updated_past = np.concatenate((obj_trajs_full[track_index_to_predict, :11, :], updated_trajs_snippet), axis=1)[:, -11:, :]
        center_obj_extrapolated_future = self.extrapolate_future_frames(center_obj_updated_past[:, -1, :], 80)
        center_obj_trajs_full_new = np.concatenate((center_obj_updated_past, center_obj_extrapolated_future), axis=1)
        # For other agents
        non_central_obj_trajs_snippet = self.extrapolate_future_frames(obj_trajs_full[track_index_non_central_agents, 10, :], updated_frames)
        non_central_obj_updated_past = np.concatenate((obj_trajs_full[track_index_non_central_agents, 11:, :], non_central_obj_trajs_snippet), axis=1)[:, :11, :]
        non_central_obj_extrapolated_future = self.extrapolate_future_frames(non_central_obj_updated_past[:, -1, :], 80)
        non_central_obj_trajs_full_new = np.concatenate((non_central_obj_updated_past, non_central_obj_extrapolated_future), axis=1)

        obj_trajs_full[track_index_to_predict, :, :] = center_obj_trajs_full_new
        obj_trajs_full[track_index_non_central_agents, :, :] = non_central_obj_trajs_full_new

        return obj_trajs_full

    def extrapolate_future_frames(self, objects_latest_features, frames_to_extrapolate):        # Asume that for the remaining frames the agents follow a constant velocity policy
        everyones_xy = objects_latest_features[:, :2]
        everyones_vel_xy = objects_latest_features[:, 7:9]
        everyones_fixed_features = objects_latest_features[:, 2:]

        future_frames_list = []
        for frame in range(frames_to_extrapolate):
            new_everyones_xy = everyones_xy + everyones_vel_xy
            new_frame = np.concatenate((new_everyones_xy, everyones_fixed_features), axis=1)
            future_frames_list.append(new_frame)
            everyones_xy = new_everyones_xy

        return np.stack(future_frames_list, axis=1)
